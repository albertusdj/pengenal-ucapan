...
canvas.width = 0.998 * window.innerWidth;
canvas.height = 0.998 * window.innerHeight;
...
function Trapezoid(x, y, dx, dy, size, color) {
  this.x = x;
  this.y = y;
  this.dx = dx;
  this.dy = dy;
  ...
  this.draw = () => {
    c.beginPath();
    c.moveTo(this.x, this.y + this.size);
    c.lineTo(this.x + this.size/4, this.y);
    c.lineTo(this.x + this.size*3/4, this.y);
    c.lineTo(this.x + this.size, this.y + this.size);
    c.lineTo(this.x, this.y + this.size);
    c.strokeStyle = this.color;
    c.stroke();
...
function addTrapezoid() {
  ...
  const color = currentTrapezoidColor;

  trapezoidArray.push(new Trapezoid(x, y, dx, dy, size, color));
}
...
function fillRectangle(color) {
  for (let i=0; i < rectangleArray.length; i++) {
    rectangleArray[i].color = color;
  }
}

function fillTriangle(color) {
  for (let i=0; i< triangleArray.length; i++) {
    triangleArray[i].color = color;
  }
}

function fillTrapezoid(color) {
  for (let i=0; i< trapezoidArray.length; i++) {
    trapezoidArray[i].color = color;
  }
}
